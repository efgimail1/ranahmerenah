...
import { formatRupiah, formatDate, toInputDate, parseCurrency, BANK_OPTIONS } from '../utils/format'
import {
  TrendingUp, TrendingDown, Wallet, Trash2,
  ArrowDownCircle, ArrowUpCircle, AlertCircle, Pencil
} from 'lucide-react'
...
const methodLabel = { cash:'Cash', transfer:'Transfer', qris:'QRIS', other:'Other' }

// ─── Income Form ───────────────────────────────────────────
function IncomeForm({ initial, onSubmit, loading, projects }) {
  const [form, setForm] = useState(initial ? {
    ...
    bank_account:'', project_id:'', is_qris:false, notes:'',
  })
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-5">
      <h4 className="text-xs font-semibold text-gray-400 uppercase tracking-widest">Income Details</h4>
      <div className="grid grid-cols-2 gap-3">
        ...
        <Input label="Received From *" value={form.received_from}
          ...
          placeholder="e.g. Mr. Budi — Villa Project" required />
        ...
        <CurrencyInput label="Gross Amount *" value={form.gross_amount}
          onChange={v => set('gross_amount', v)} placeholder="0" />
...
function ExpenseForm({ initial, onSubmit, loading, projects }) {
  const [form, setForm] = useState(initial ? {
    ...
    gross_expense:     initial.gross_expense     ? String(Math.round(parseFloat(initial.gross_expense))) : '',
    discount_received: initial.discount_received ? String(Math.round(parseFloat(initial.discount_received))) : '',
    payment_method:    initial.payment_method    || 'cash',
    bank_account:      initial.bank_account      || '',
    project_id:        initial.project_id        || '',
    notes:             initial.notes             || '',
  } : {
    entry_date:'', description:'', paid_to:'',
    gross_expense:'', discount_received:'',
    payment_method:'cash', bank_account:'', project_id:'', notes:'',
  })
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-5">
      <h4 className="text-xs font-semibold text-gray-400 uppercase tracking-widest">Expense Details</h4>
      <div className="grid grid-cols-2 gap-3">
        <Input label="Date *" type="date" value={form.entry_date}
          onChange={e => set('entry_date', e.target.value)} required />
        <Input label="Paid To" value={form.paid_to}
        ...
        <CurrencyInput label="Gross Amount (before discount)" value={form.gross_expense}
          onChange={v => set('gross_expense', v)} placeholder="0" />
        <CurrencyInput label="Discount Received" value={form.discount_received}
          onChange={v => set('discount_received', v)} placeholder="0" />
        <Input label="Notes" value={form.notes}
          onChange={e => set('notes', e.target.value)}
          placeholder="optional" className="col-span-2" />
      </div>
...
export default function Ledger() {
  ...
  const { data: summary } = useQuery({
    ...
    queryFn: () => ledgerApi.getSummary(
      filterProject || dateFrom || dateTo
        ? { project_id: filterProject || undefined, date_from: dateFrom || undefined, date_to: dateTo || undefined }
        : {}
    ),
  })
  ...
  const incomeMutation = useMutation({
    ...
    onSuccess: () => { inv(); setModalType(null); toast.success('Income recorded!') },
    onError:   (e) => toast.error(e.message),
  })

  const expenseMutation = useMutation({
    mutationFn: ledgerApi.createExpense,
    onSuccess: () => { inv(); setModalType(null); toast.success('Expense recorded!') },
    onError:   (e) => toast.error(e.message),
  })

  const updateMutation = useMutation({
    mutationFn: ({ id, data }) => ledgerApi.update(id, data),
    onSuccess: () => { inv(); setModalType(null); setEditEntry(null); toast.success('Entry updated!') },
    onError:   (e) => toast.error(e.message),
  })

  const deleteMutation = useMutation({
    mutationFn: ledgerApi.delete,
    onSuccess: () => { inv(); toast.success('Entry deleted.') },
    onError:   (e) => toast.error(e.message),
  })
    ...
    <div className="space-y-5">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-0.5">Income & expense tracking</p>
        </div>
      ...
      <div className="grid grid-cols-4 gap-3">
        ...
        <div className={`border rounded-xl p-4 ${netBalance>=0?'bg-blue-50 border-blue-200':'bg-orange-50 border-orange-200'}`}>
          <div className="flex items-center gap-2 mb-2">
            ...
            <span className={`text-xs font-medium ${netBalance>=0?'text-blue-600':'text-orange-600'}`}>Net Balance</span>
          </div>
      ...
      <div className="flex items-center gap-3 flex-wrap">
        <div className="flex gap-1.5">
          {[{l:'All',v:''},{l:'Income',v:'income'},{l:'Expense',v:'expense'}].map(f => (
            <button key={f.v} onClick={() => setFilterType(f.v)}
              className={`px-3 py-1.5 rounded-full text-xs font-medium transition-all ${
                filterType===f.v ? 'bg-emerald-600 text-white' : 'bg-white text-gray-600 border border-gray-200 hover:bg-gray-50'
              }`}>{f.l}</button>
          ))}
        </div>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-gray-200 bg-gray-50">
                  <th className="text-left px-4 py-3 text-xs font-medium text-gray-500 uppercase tracking-wide w-28">Date</th>
                  ...
                  <th className="text-left px-4 py-3 text-xs font-medium text-gray-500 uppercase tracking-wide w-20">Bank</th>
                  <th className="text-left px-4 py-3 text-xs font-medium text-gray-500 uppercase tracking-wide w-28">Project</th>
                  <th className="text-right px-4 py-3 text-xs font-medium text-gray-500 uppercase tracking-wide w-32">Gross</th>
                  <th className="text-right px-4 py-3 text-xs font-medium text-gray-500 uppercase tracking-wide w-32">Net</th>
                  <th className="text-left px-4 py-3 text-xs font-medium text-gray-500 uppercase tracking-wide w-20">Type</th>
                  ...
                  return (
                    <tr key={entry.id} className="hover:bg-gray-50 transition-colors">
                      ...
                      <td className="px-4 py-3">
                        ...
                        {entry.is_qris && (
                          <div className="text-xs text-amber-600">QRIS fee: {formatRupiah(entry.qris_fee_amount)}</div>
                        )}
                        {!isIncome && parseFloat(entry.discount_received||0) > 0 && (
                          <div className="text-xs text-emerald-600">Discount: {formatRupiah(entry.discount_received)}</div>
                        )}
                      </td>
                      ...
                      <td className="px-4 py-3">
                        {entry.bank_account
                          ? <span className="text-xs font-medium text-gray-700 bg-gray-100 rounded px-1.5 py-0.5">{entry.bank_account}</span>
                      ...
                      <td className="px-4 py-3 text-right text-gray-700 text-sm">
                        {formatRupiah(isIncome ? entry.gross_amount : entry.gross_expense)}
                      </td>
                      ...
                      <td className="px-4 py-3">
                        <div className="flex items-center gap-1">
                          ...
                          <button onClick={() => { if (confirm('Are you sure you want to delete this entry?')) deleteMutation.mutate(entry.id) }}
              ...
              <tfoot>
                <tr className="border-t-2 border-gray-200 bg-gray-50">
                  ...
                  <td className="px-4 py-3 text-right text-sm font-semibold text-gray-700">
                    {formatRupiah(entries.reduce((s,e) => s + parseFloat(e.entry_type==='income' ? e.gross_amount||0 : e.gross_expense||0), 0))}
                  </td>
      ...
      {/* Modals */}
      <Modal open={modalType==='income'} onClose={() => setModalType(null)}
        title="Record Income" size="md">
        <IncomeForm key="income" onSubmit={d => incomeMutation.mutate(d)}
          loading={incomeMutation.isPending} projects={projects} />
      </Modal>

      <Modal open={modalType==='expense'} onClose={() => setModalType(null)}
        title="Record Expense" size="md">
        <ExpenseForm key="expense" onSubmit={d => expenseMutation.mutate(d)}
          loading={expenseMutation.isPending} projects={projects} />
      </Modal>

      <Modal open={modalType==='edit' && !!editEntry} onClose={() => { setModalType(null); setEditEntry(null) }}
        title="Edit Entry" size="md">
        {editEntry?.entry_type === 'income' ? (
          <IncomeForm key={editEntry?.id} initial={editEntry}
            onSubmit={handleEditSubmit}
            loading={updateMutation.isPending} projects={projects} />
        ) : (
          <ExpenseForm key={editEntry?.id} initial={editEntry}
            onSubmit={handleEditSubmit}
            loading={updateMutation.isPending} projects={projects} />
        )}
      </Modal>
